...
def data_merge_result(result_pkl, work_dir, epoch, name, dataset_type, images_dir=""):
    assert dataset_type in ["FAIR", "DOTA", "DOTA1_5",
                            "DOTA2"], "need to set dataset.test.dataset_type in the config file. FAIR, DOTA, DOTA1_5 and DOTA2 are supported"
    print("Merge results...")
    ...
    if (dataset_type == 'FAIR'):
        ...
        final_fair_path = os.path.join(
            work_dir, f"test/submit_{epoch}/final_fair/test")
    ...
    if (dataset_type == 'FAIR'):
        if is_win():
            ...
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as t:
                for f in files:
                    t.write(f, os.path.join(
                        "test", os.path.split(f)[-1]))  # TODO
        else:
            os.system(
                f"cd {os.path.join(final_path, '..')} && zip -r -q {name+'.zip'} 'test'")
            os.system(
                f"mv {os.path.join(final_path, '..', name+'.zip')} {zip_path}")
    else:
        if is_win():
            files = glob.glob(os.path.join(final_path, "*"))
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as t:
...
if __name__ == "__main__":
    work_dir = "JDet/work_dirs/gliding_r101_fpn_1x_dota_bs2_tobgr_steplr_rotate_balance_ms"
    epoch = 12
