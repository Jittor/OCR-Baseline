...
class Runner:
    def __init__(self):
        ...
        if (cfg.parameter_groups_generator):
            params = build_from_cfg(cfg.parameter_groups_generator, MODELS,
                                    named_params=self.model.named_parameters(), model=self.model)
        else:
        ...
        self.train_dataset = build_from_cfg(
            cfg.dataset.train, DATASETS, drop_last=jt.in_mpi)
    ...
    def run_on_images(self, save_dir=None, **kwargs):
        ...
        for i, (images, targets) in tqdm(enumerate(self.test_dataset)):
            ...
            if save_dir:
                visualize_results(sync(results), get_classes_by_name(self.test_dataset.dataset_type), [
                                  t["img_file"] for t in targets], save_dir, **kwargs)

    @jt.no_grad()
    @jt.single_process_scope()
    def run_on_one_image(self, image, target):
        self.model.eval()
        results = self.model(image, target)
        return results
        ...
        if self.test_dataset is None:
            ...
            for batch_idx, (images, targets) in tqdm(enumerate(self.test_dataset), total=len(self.test_dataset)):
                ...
                for mode in self.flip_test:
                    ...
                    results.extend(
                        [(r, t) for r, t in zip(sync(result), sync(targets_))])
            ...
            if (self.cfg.dataset.test.type == "ImageDataset"):
                ...
                data_merge_result(save_file, self.work_dir, self.epoch,
                                  self.cfg.name, dataset_type, self.cfg.dataset.test.images_dir)
    ...
    def load(self, load_path, model_only=False):
        ...
        if (not model_only):
            ...
            self.scheduler.load_parameters(
                resume_data.get("scheduler", dict()))
            self.optimizer.load_parameters(
                resume_data.get("optimizer", dict()))
