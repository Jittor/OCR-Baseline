...
def visualize_results(results, classnames, files, save_dir, **kwargs):
    ...
    for (bboxes, scores, labels), img_file in tqdm(zip(results, files)):
        ...
        draw_bboxes(img_file, bboxes, labels=labels, scores=scores,
                    class_names=classnames, out_file=save_file, **kwargs)
...
def visualize_dota_ground_truth(gt_dir, classnames, save_dir, style=0):
    ...
    if style == 1:
        ...
        visualize_results(results, classnames, files, save_dir,
                          colors=DOTA_COLORS, with_text=False, thickness=2)


def main():
    gt_dir = "workspace/JAD/datasets/DOTA/train"
    save_dir = "./temp"
    visualize_dota_ground_truth(gt_dir, DOTA1_CLASSES, save_dir, style=1)

    # dota_dir = "workspace/JAD/SAR/JDet/projects/gliding/submit_zips/temp2"
    # image_dir = "workspace/JAD/datasets/DOTA/test/images"
    # save_dir = "./temp"
