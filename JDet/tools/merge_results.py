...
def merge_files(src_path, dst_path, nms_op=py_cpu_nms_poly_fast, nms_thr=0.1, process_num=37):
    ...
    merge_fn = partial(merge_file, dst_path=dst_path,
                       nms_op=nms_op, nms_thr=nms_thr)
    if process_num < 2:
...
def main():
    ...
    name = "fair_merge16"
    image_dirs = "dataset/fair_1024/test_1024_200_0.5-1.0-1.5/images/"
    dataset_type = "fair"

    save_dir = "temp"
    merge_results(result_dirs, save_dir, name=name,
                  image_dirs=image_dirs, dataset_type=dataset_type)
